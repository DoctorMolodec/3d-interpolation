...
Scene3D::Scene3D(int method, int n_x, int n_y, int function_number, double x_a, double x_b, double y_a, double y_b)
...
void Scene3D::drawFigure() // build the figure
{
    ...
    painter.drawText(0, 20, "Next graph: 1");
    ...
    painter.drawText(500, 20, "Rotate -15: 8");
    painter.drawText(600, 20, "Rotate +15: 9");
    painter.drawText(700, 20, "Next function: 0");
    painter.drawText(800, 20, "Exit: Esc");

    painter.setPen(Qt::blue);
    painter.drawText(0, 40, function_name);
    painter.drawText(0, 60, "graph = " + QString::number(graph_number));
    painter.drawText(0, 80, "n_x = " + QString::number(n_x));
    painter.drawText(0, 100, "n_y = " + QString::number(n_y));
    painter.drawText(0, 120, "scale = " + QString::number(scale));
    painter.drawText(0, 140, "angle = " + QString::number(zRot - 360 * floor(zRot / 360)) + " degree");
    painter.setPen(Qt::red);
    painter.drawText(0, 160, "max = " + QString::number(sup));
    painter.drawText(0, 180, "min = " + QString::number(inf));
    painter.drawText(0, 200, "x: (" + QString::number(x_a1) + ", " + QString::number(x_b1) + ")");
    painter.drawText(0, 220, "y: (" + QString::number(y_a1) + ", " + QString::number(y_b1) + ")");
    if ((graph_number == 3) || (graph_number == 4))
    {
        painter.drawText(0, 240, "Error drop: " + QString::number(error_drop));
    }
    painter.end();
}
